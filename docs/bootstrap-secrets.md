...
- `authentik-bootstrap` seeds the admin bootstrap password/token on first startup (set it before Argo sync).
...
After authentik is running, you can either reuse the bootstrap token or create a separate API token and store it as a secret:
